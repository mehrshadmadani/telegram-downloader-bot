...
class TelethonWorker:
    ...
    def _try_instaloader(self, url, code):
        ...
        dl_folder = os.path.join(self.download_dir, f"{code}_temp"); downloaded_files = []
        ...
        if downloaded_files:
            ...
            os.rmdir(dl_folder)
            return (downloaded_files, caption, "instaloader")
        return ([], None, None)
    ...
    async def upload_single_file(self, message, file_path, code, user_id, download_method, index, total, original_caption):
        ...
        try:
            ...
            return True # بازگرداندن موفقیت
        except Exception as e:
            logger.error(f"Upload failed for {file_path}: {e}")
            return False # بازگرداندن شکست
        finally:
    ...
    async def process_job(self, message):
        ...
        if file_paths:
            ...
            # --- منطق اصلاح شده برای بررسی نتایج ---
            results = await asyncio.gather(*upload_tasks)
            # اگر حتی یکی از نتایج False (ناموفق) بود، کل کار را ناموفق بدان
            if False in results:
