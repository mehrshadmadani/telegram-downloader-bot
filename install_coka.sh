...
# =============================================================
#         Coka Bot Manager - Universal Smart Installer v6.0
# =============================================================
...
if [ -f "$COKA_SCRIPT_PATH" ]; then
    ...
    read -p "Do you want to force overwrite it with the latest version (Interactive Menu)? (y/n): " OVERWRITE_CONFIRM
...
# --- Writing the coka script content (Interactive Menu Version) ---
cat > "$COKA_SCRIPT_PATH" << EOF
#!/bin/bash
VERSION="6.0 (Interactive Menu)"
BOT_DIR="$BOT_DIR"
...
show_menu() {
    ...
    echo "  [5] Update Manager (Update Kardan-e Coka)"
    echo "  [q] Quit (Khorooj)"
    echo -e "\e[2m----------------------------------------------------------\e[0m"
}
...
while true; do
    ...
    case \$choice in
        ...
        2)
            ...
            ;;
        3)
            print_info "Attaching to live dashboard... Press Ctrl+A then D to detach."
            sleep 2
...
print_success "Management script 'coka' (v6.0 - Interactive) installed successfully!"
echo
