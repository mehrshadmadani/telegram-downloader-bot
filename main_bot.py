# ... (تمام بخش‌های بالای فایل تا کلاس AdvancedBot بدون تغییر) ...
...
from config import (BOT_TOKEN, GROUP_ID, DB_NAME, DB_USER, DB_PASS, DB_HOST, DB_PORT, ORDER_TOPIC_ID, LOG_TOPIC_ID, ADMIN_IDS, FORCED_JOIN_CHANNELS)

class PostgresDB:
    def __init__(self): self.conn_params = {"dbname": DB_NAME, "user": DB_USER, "password": DB_PASS, "host": DB_HOST, "port": DB_PORT}; self.init_database()
    def get_conn(self): return psycopg2.connect(**self.conn_params)
    ...
    def add_user_if_not_exists(self, user: Update.effective_user):
        with self.get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute('INSERT INTO users (user_id, first_name, username) VALUES (%s, %s, %s) ON CONFLICT (user_id) DO NOTHING;', (user.id, user.first_name, user.username)); return cur.rowcount > 0
    def add_job(self, code, user_id, url):
        with self.get_conn() as conn:
            with conn.cursor() as cur: cur.execute("INSERT INTO jobs (code, user_id, url) VALUES (%s, %s, %s);", (code, user_id, url))
    def get_user_by_code(self, code):
        with self.get_conn() as conn:
            with conn.cursor() as cur: cur.execute("SELECT user_id FROM jobs WHERE code = %s;", (code,)); result = cur.fetchone(); return result[0] if result else None
    def update_job_on_complete(self, code, status, file_size):
        with self.get_conn() as conn:
            with conn.cursor() as cur: cur.execute("UPDATE jobs SET status = %s, completed_at = NOW(), file_size = %s WHERE code = %s;", (status, file_size, code))
...
class AdvancedBot:
    ...
    async def handle_url(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not self.is_valid_url(url):
            await update.message.reply_text("❌ لینک نامعتبر است. لطفاً لینک معتبر ارسال کنید.")
            return
    ...
    async def handle_group_files(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            elif update.message.audio: await context.bot.send_audio(chat_id=user_id, audio=update.message.audio.file_id, caption=caption, parse_mode='Markdown')
    ...
    def run(self):
        ...
        print("🚀 Bot is running with Multi-Platform Support..."); self.app.run_polling()
