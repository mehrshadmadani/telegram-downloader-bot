...
class AdvancedBot:
    ...
    async def handle_group_files(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            file_id = getattr(update.message, media_type).file_id if media_type not in ['photo'] else getattr(update.message, media_type)[-1].file_id
            ...
            if len(decoded_caption) > 1000:
                ...
                for i in range(0, len(decoded_caption), 4096):
                    await context.bot.send_message(chat_id=user_id, text=decoded_caption[i:i+4096], disable_web_page_preview=True)
            else:
                full_caption = (decoded_caption + slide_info + footer).strip()
